...
def has_winfsp():
    ...
    try:
        with winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, service_path, 0, winreg.KEY_READ
        ) as key:
            return True
...
print(
    f"{ansi("yellow",0)}>> GRACE RUNTIME AUDIO PATCHER FOR ROBLOX ( Patch VII: dementia )"
)
...
with open(".\\lib\\db.txt") as f:
    ...
    for l in lines:
        ...
        if (len(parts) == 3 and parts[2].lower() == "killcode"):
            killList.append(targ)
...
with open(".\\lib\\db_hashes.txt") as f:
    lines = f.readlines()
    for l in lines:
        if len(l) <= 1:
            continue
...
class GraceOperations(InMemoryFileSystemOperations):
    def write(self, file_context, buffer, offset, write_to_end_of_file, constrained_io):
        ...
        hsh = xxhash.xxh3_64_hexdigest(buffer)

        if DEBUG:
            trueName = os.path.basename(file_context.file_obj.path)
            print(f"+debug | {trueName} -> {fsize} bytes | hsh={hsh}")
            size_mapping[trueName] = {"sz": fsize, "hash": hsh}
        ...
        if (fsize in killList or hsh in killList) and not recurse_blocker:
            ...
            print(
                f"{ansi("red",0)}[KILLCODE] Shimmed {ansi("cyan",0)} {targetmap[hsh] if hsh in targetmap else targetmap[fsize]}" + ansi("rst", 0)
            )
            recurse_blocker = False
        elif (fsize in targets or hsh in targets) and not recurse_blocker:
            ...
            with open(up_next, "rb") as file:
                ...
                file_context.file_obj.data = data
                print(
                    f"{ansi("green",0)}[SUCCESS] Piped cache file! :) {ansi("blue",0)} Up next: "
                    + os.path.basename(up_next)
                    + ansi("rst", 0)
                )
                reroll_music()
            recurse_blocker = False
...
def main(mountpoint):
    ...
    try:
        ...
        print(
            "Some exception (manual or error) has occured, and the Grace Audio Patcher will now exit."
        )
    finally:
